...
function typetester(e, font, size, tracking, italic,  weight, opt){
	...
	if (sizeoptions !== ""){
		...
		if ($.inArray(sizeoptions, no) >= 0){
			...
			sizeoptions = "<div class='option slider sizeSlider sizeSlider" + testerNumber + " size'><span class='label'>size</span><span class='amount'></span></div>";
		}
	...
	if (trackingoptions !== ""){
		...
		if ($.inArray(trackingoptions, no) >= 0){
			...
			trackingoptions = "<div class='option slider trackingSlider trackingSlider" + testerNumber + " tracking'><span class='label'>tracking</span><span class='amount'></span></div>";
	...
	if (weightoptions !== ""){
		...
		if ($.inArray(weightoptions, no) >= 0){
			...
			weightoptions = "<div class='option slider weightSlider weightSlider" + testerNumber + " weight'><span class='label'>weight</span><span class='amount'></span></div>";
	...
	if (italicoptions !== ""){
		...
		if ($.inArray(italicoptions, no) >= 0){
			...
			italicoptions = "<div class='option button italicButton italicButton" + testerNumber + " italic'><span class='label'>italic</span></div>";
	...
	if (optoptions !== ""){
		...
		if ($.inArray(optoptions, no) >= 0){
			...
			optoptions = "<div class='option optButton optButton" + testerNumber + " opt' active='features'><span class='label'>features</span><div class='dropdown'>" +testerOpt +"</div></div>";
	...
	if (size !== ""){
		...
		if (alignoptions !== ""){
			...
			if ($.inArray(alignoptions, no) >= 0){
				...
				alignoptions = "<div class='option slider alignSlider alignSlider" + testerNumber + " align'><span class='label'>align</span><span class='amount'></span></div>";
			}
		}
	}
	...
	if (size !== ""){
		...
		if (singlelineoptions !== ""){
			...
			if ($.inArray(singlelineoptions, no) >= 0){
				...
				singlelineoptions = "<div class='option button singlelineButton singlelineButton" + testerNumber + " singleline'><span class='label'>singleline</span></div>";
	...
	if (options !== "false"){
		testeroptions = "<div class='options row width" + elm + "'>" +
				totaloptions + optDisclaim +
			"</div>"
	}

	typetester = "<div class='container " + testerfit + "'><div class='type row' style='" + testerFont + testerSize + testerTracking + testerWeight + testerItalic + testerAlign + "''><span class='enterTXT' spellcheck='false' contenteditable style='" + testerSingleline + "'>" + text + "</span></div></div>" + testeroptions;
